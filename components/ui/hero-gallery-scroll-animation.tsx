...
const BentoCell = React.forwardRef<HTMLDivElement, HTMLMotionProps<'div'>>(
  ({ className, style, ...props }, ref) => {
    ...
    const translate = useTransform(scrollYProgress, [0.05, 0.8], ['-25%', '0%']);
...
const ContainerScale = React.forwardRef<HTMLDivElement, HTMLMotionProps<'div'>>(
  ({ className, style, ...props }, ref) => {
    ...
    const scale = useTransform(scrollYProgress, [0, 0.45], [1, 0.8]);

    const position = useTransform(scrollYProgress, (pos) =>
      pos >= 0.55 ? 'absolute' : 'fixed'
    );

    return (
      <motion.div
        ref={ref}
        className={cn('left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 size-fit z-20', className)}
        style={{
          scale,
          position,
          opacity,

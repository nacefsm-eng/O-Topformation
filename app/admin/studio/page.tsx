...
export default function ContentStudio() {
  ...
  const handleGenerate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      try {
        let cleanResult = data.result.replace(/```json/gi, '').replace(/```/g, '').trim();
        const parsedData = JSON.parse(cleanResult);
        if (parsedData.posts) {
          setResult(parsedData.posts);
        } else {
          setResult([]);
          setError("Format de réponse inattendu.");
        }
      } catch (e) {
        setError("Erreur de formatage de l'IA. Veuillez réessayer.");
        console.error("Parse error:", data.result);
      }
    } catch (err: any) {
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '2rem' }}>
          ...
          <div style={{ background: 'var(--blue-900)', color: 'white', padding: '2rem', borderRadius: 'var(--radius)', boxShadow: 'var(--shadow-lg)', display: 'flex', flexDirection: 'column' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
              <h2 style={{ fontSize: '1.25rem', color: 'var(--gold)' }}>Résultats Générés</h2>
            </div>
            
            <div style={{ flex: 1, background: 'rgba(0,0,0,0.2)', padding: '1.5rem', borderRadius: 'var(--radius-sm)', overflowY: 'auto', maxHeight: '600px' }}>
              {loading ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '2rem' }}>
                  {result.map((post, idx) => (
                    <div key={idx} style={{ background: 'rgba(255,255,255,0.05)', padding: '1.5rem', borderRadius: '8px', border: '1px solid rgba(255,255,255,0.1)' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
                        <h3 style={{ fontSize: '1.1rem', color: 'var(--gold)' }}>{post.platform}</h3>
                        <button onClick={() => handleCopy(post.text)} style={{ background: 'rgba(255,255,255,0.1)', color: 'white', border: '1px solid rgba(255,255,255,0.2)', padding: '0.4rem 0.8rem', borderRadius: '4px', cursor: 'pointer', fontSize: '0.8rem', transition: 'background 0.2s' }}>
                          📋 Copier
                        </button>
                      </div>
                      
                      <div style={{ fontSize: '0.95rem', lineHeight: 1.6, whiteSpace: 'pre-wrap', marginBottom: '1.5rem' }}>
                        {post.text}
                      </div>

                      {post.imagePrompt && (
                        <div>
                          <div style={{ fontSize: '0.8rem', color: 'rgba(255,255,255,0.5)', marginBottom: '0.5rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>Suggestion d'image générée :</div>
                          <img 
                            src={`https://image.pollinations.ai/prompt/${encodeURIComponent(post.imagePrompt)}?width=800&height=600&nologo=true`} 
                            alt="Generated by AI" 
                ...
                <span style={{ color: 'rgba(255,255,255,0.5)' }}>Vos posts et images apparaîtront ici...</span>
              )}
            </div>
          </div>
        </div>
